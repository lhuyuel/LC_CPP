int lengthLongestPath(string input) {
    ...
    for (int i = 0; i < n; ) {
        ...
        if (toVisit.empty() || depth > toVisit.top().first) {
            if (isFile ) {
                if (curLen + depth > maxLen) {
                    maxLen = curLen + depth;
                }
                curLen -= name.length();
            }
        ...
        // if it is at the same depth, that means we have exhausted all subdirs, pop from stack
            ...
            // trace back to the same depth
            while (!toVisit.empty() && depth <= toVisit.top().first) {
                curLen -= toVisit.top().second.length() ;
                toVisit.pop();
